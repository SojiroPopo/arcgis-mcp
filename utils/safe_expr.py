...
SA_FUNCTION_WHITELIST: Set[str] = {
    "Raster", "Con", "Pick", "SetNull", "IsNull",
    "Abs", "Exp", "Exp10", "Exp2", "Ln", "Log10", "Log2", "Power", "Square", "SquareRoot",
    "Int", "Float", "RoundDown", "RoundUp", "Mod", "Negate",
    "CellStatistics", "FocalStatistics", "Slope", "Aspect", "Hillshade", "Fill",
    "Reclassify", "Lookup", "Test", "Diff", "InList", "Over",
    "BooleanAnd", "BooleanOr", "BooleanNot", "BooleanXOr",
    "Cos", "Sin", "Tan", "ACos", "ASin", "ATan", "ATan2",
    "Plus", "Minus", "Times", "Divide",
    "EqualTo", "NotEqual", "GreaterThan", "GreaterThanEqual", "LessThan", "LessThanEqual",
    "NbrRectangle", "NbrCircle", "NbrAnnulus", "NbrWedge", "NbrIrregular", "NbrWeight",
    "RemapValue", "RemapRange",
}
...
_ALLOWED_NODES = (
    ...
    ast.Call, ast.Name, ast.Constant, ast.Load, ast.List, ast.Tuple, ast.keyword,
    # operators
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Name):
            if node.id not in allowed or node.id.startswith("_"):
                raise UnsafeExpressionError(
                    f"Unknown or disallowed name: {node.id!r}. "
                    "Use Raster(...) and arcpy.sa functions only."
                )
